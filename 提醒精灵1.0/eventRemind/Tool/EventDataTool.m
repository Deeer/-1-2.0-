...
+(void)initialize
{
    ...
    if ([_db open])
    {
        BOOL result = [_db executeUpdate:@"create table if not exists t_event (id integer primary key autoincrement,text2 text,music text,date blob,idenity text,new text,handOff text,times text,groupInfo text,access_token text);"];
...
+(void)addDBModel:(remainModel *)model
{
    ...
    BOOL result = [_db executeUpdate:@"insert into t_event (text2,idenity,music,date,access_token,new,handOff,times,groupInfo) values (?,?,?,?,?,?,?,?,?)",model.text,model.idenity,model.music,model.date,model.access_token,[NSString stringWithFormat:@"%hhd",model.New],[NSString stringWithFormat:@"%hhd",model.handOff],[NSString stringWithFormat:@"%d",model.timesNum],model.groupInfo];
...
{
    BOOL result = [_db executeUpdate:@"delete from t_event where idenity = ? and access_token = ? ",model.idenity,model.access_token];
...
+(void)modifyDBModel:(remainModel *)model
{
    ...
    BOOL result = [_db executeUpdate:@"update t_event set text2 = ?,idenity = ? ,music = ? ,date = ?, new = ?, handOff = ? ,times = ? ,groupInfo = ?  where access_token = ? ",model.text,model.idenity,model.music,model.date,[NSString stringWithFormat:@"%hhd",model.New],[NSString stringWithFormat:@"%hhd",model.handOff],[NSString stringWithFormat:@"%d",model.timesNum],model.groupInfo,model.access_token];
